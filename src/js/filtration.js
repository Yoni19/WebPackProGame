import {platformUrl} from "./index";

const Filter = (thisWeekArgument, orderByRating) => {
  fetch(platformUrl)
    .then((response) => response.json())
    .then((response) => {
      const filter = document.getElementById("platform_filter");
      let results = response.results;
      let innerHTML = `<option selected="true" disabled="disabled">--Choisir une plateforme--</option>`;
